...
const TechStackIcon = ({ name, size = 40 }: { name: string; size?: number }) => {
    const iconKey = `si${name.toLowerCase().replace(/[/.]/g, '').replace(/\s+/g, '')}`
    ...
    return (
      <div 
        className="group relative w-16 h-16 flex items-center justify-center bg-dark-light rounded-lg 
      ...
      >
        ...
        />
        <span className="absolute -bottom-8 px-2 py-1 bg-dark-light rounded text-sm text-white 
  ...
  const TechStackCarousel = () => {
    const techs = [
      ...
      "python", "postgresql", "tailwindcss", "git", "solidity",
    ...
    return (
      <div className="relative w-full overflow-hidden bg-dark-light/20 py-12 my-16">
        <div className="tech-slider flex">
          {techs.map((tech, index) => {
            const iconKey = `si${tech}`
            const icon = SimpleIcons[iconKey]
...
const skills = [
  ...
  },
  {
    ...
    skills: ["Node.js", "Python", "RESTful APIs", "SQLite", "PostgreSQL"]
  },
  {
